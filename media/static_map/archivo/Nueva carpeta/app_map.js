...
global.selectionToolbar_ag;
 ...
 global.url_ze_urbana ="https://geosisfoh.midis.gob.pe/server/rest/services/Geosisfoh/PERU_MIDIS_001_DGFIS_DU/MapServer/1";
 global.url_agsisfoh ="https://geosisfoh.midis.gob.pe/server/rest/services/Geosisfoh/PERU_MIDIS_1806_DGFIS_DU/MapServer/0";
 global.service_ccpp="https://geosisfoh.midis.gob.pe/server/rest/services/Geosisfoh/PERU_INEI_Cartografia_Base/MapServer/1";
 ...
 global.url_report_du ="https://geosisfoh.midis.gob.pe/server/rest/services/Geosisfoh/PERU_MIDIS_1806_DGFIS_DU/MapServer";
    ...
    function(
      ...
      function _geolocaliza_xy_(id,lat,long,direccion){
       ...
       function cargar_data(){
        const serviceURL =global.url_report_du+"/4";
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            const select = $('#filtroGeo');
             ...
             select.append("<option value='0'>Todos los Georref...</option>");
...
var layer_cenepred, layer_base, layer_estrato,featureLayer_ze, featureLayer_PGH;
...
function addInitialLayers(map) {
    ...
    map.addLayers([layer_cenepred, layer_estrato, layer_base, featureLayer_PGH,featureLayer_ze]);
}
...
function setupLayerList(map) {
 ...
 on(llWidget, 'toggle', function(evt){
    ...
    if(evt.layerIndex==1 && evt.visible==true){
      selectcapas.append("<option value='1'>PGH Georreferenciado</option>");
    }
    else if(evt.layerIndex==1 && evt.visible==false){       
      $('#filtrocapas option[value="1"]').remove();
    ...
    else if(evt.layerIndex==3 && evt.subLayerIndex==0 && evt.visible==true){
      const valorAInsertar = '3'; // Valor a insertar en el nuevo option
    ...
    else if(evt.layerIndex==3 && evt.visible==false){
      $('#filtrocapas option[value="3"]').remove();
      $('#filtrocapas').prop('selectedIndex', 0);
      $("#ReporteInfortabla_estratos").hide();
      $("#ReporteInfortabla_pgh_geo").hide();
      $("#herramientas_selec").hide();
  ...
  /*******************************************/       
   ...
   $("#ccprov").change(function() {
    ...
    $("#ccpoblado").change(function() {
       var ccpoblado=$(this).val();
             set_graphic({
                    ...
                    where: "IDCCPP_21 = '" + ccpoblado + "'",
                    template_title: "Ubicación Geográfica",
                    template_body: "<div style='font: 14px Segoe UI'><b>Ubigeo :</b> ${UBIGEO}<br><b>Departamento :</b> ${NOMBDEP}<br><b>Provincia :</b> ${NOMBPROV}<br><b>Distrito :</b> ${NOMBDIST}<br><b>CENTRO POBLADO:</b> ${NOMBCCPP}</div>",
                    zoom: 16
                });
            });
    ...
    /********************************************************/
     ...
     function cargarpension65(ubigs){

        $('#datos_load_p65').show();

       var infoTemplate = new InfoTemplate("DNI: ${num_doi}", "<div><b>Nombres y Apellidos:</b>${nomb} ${ap_pat} ${ap_mat}<br/>");

       featureLayer_pension = new FeatureLayer(global.url_P65,
              {
        ...
        if (ubigs!==0) {
          featureLayer_pension.setDefinitionExpression("ubigeo='"+ubigs+"'");
        }

      var defaultSymbol = new SimpleMarkerSymbol().setColor(new Color([52, 152, 219,0.9])).setSize(6);
      var renderer = new ClassBreaksRenderer(defaultSymbol, "clusterCount");

      var small = new SimpleMarkerSymbol().setColor(new Color([52, 152, 219,0.5])).setSize(15);
      //var medium = new SimpleMarkerSymbol().setColor(new Color([52, 152, 219,0.5])).setSize(30);
       renderer.addBreak(1, 1000, small);
       //renderer.addBreak(101, 1000, medium);

      featureLayer_pension.setRenderer(renderer);
      map.addLayer(featureLayer_pension);

     toggleFeatureReduction(true);

      function toggleFeatureReduction(yes) {
        if (yes) {
          if (!featureLayer_pension.getFeatureReduction()) {
            featureLayer_pension.setFeatureReduction({
              type: "cluster",
              infoTemplate: new PopupTemplate({
                  title: "Información",
                  description: "Cantidad de usuarios en el cluster es: {cluster_count}"
                })
              
            });
          } else {
            featureLayer_pension.enableFeatureReduction();
          }
        } else {
          featureLayer_pension.disableFeatureReduction();
        }
      }
     ...
     function deletepension65(){

      map.removeLayer(featureLayer_pension);
     }
    ...
    function cargaragsisfoh(sele,ubig){ 
      ...
      document.getElementById("dato-valor").textContent ="";      
         ...
         var seleText = {
          ...
          featureLayer_agsisfoh.setDefinitionExpression(definid_query);
              ...
              var query = new Query();
              query.where = definid_query;
              featureLayer_agsisfoh.queryFeatures(query, function (result) {
              ...
              document.getElementById("dato-valor").textContent = numRegistros;
    ...
    function deleteagsisfoh(){
     ...
     var selectElement = document.getElementById("filtroGeo");
        ...
        selectElement.addEventListener("change", function() {
            ...
            if ($('#ccdist').val() !== valorVacio) {
              var ubig=$('#ccdist').val();
              cargaragsisfoh(selectedValue, ubig);
            }
